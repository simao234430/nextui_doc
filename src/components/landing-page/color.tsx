...
{scale && (
          <span
            className="color-palette-item-left text-tiny font-semibold"
            style={{
        ...
        <span
          className="opacity-0  color-palette-value text-small font-semibold"
          style={{
            color: readableColor(color),
          }}
        >
  ...
  <div style={{ textAlign: 'center'}} className="flex flex-row flex-wrap items-center justify-center">
    {colors.map(({title, items}) => (
      <div key={title} className="">
        ...
        <div className="color-palette-wrapper flex group  flex-col flex-wrap items-center justify-start w-full h-full py-1">
          {items.map((c, index) =>
            isSematic ? (
              <SematicSwatch
                key={`${c.color}-${index}`}
  ...
  return (
    <SwatchCard
      colors={[
        ...
        {
          title: "arcoBlue",
          items: getCommonItems([...Object.values(commonColors.arcoblue)]),
        },
        {
          title: "Blue",
